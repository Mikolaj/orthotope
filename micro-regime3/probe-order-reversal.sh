...
#
# Artifacts probe-ord-<class>-<slot>-<class>.{json,log}, where <slot> is
...
# appended to probe-order-reversal.txt as it lands; the last line is the
# one to wait on.
set -u
cd "$(dirname "$0")" || exit 1
...
stage () {  # class, binary, slot name
  ...
  BIN=$bin OUT=probe-ord-$cls-$slot ./probe-times.sh "$cls"
  local rc=$?
...
log "ORDER REVERSAL COMPLETE: complaints=$BAD -- read the '!!' lines of each\
 probe-ord-*-wallclock.log anyway, then each class's floor and level in both\
 orders with ./read-run.py probe-ord-<class>-<slot>-<class>.json --block and\
