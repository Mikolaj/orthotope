...
with its dimension lists replaced by unboxed vectors --- is on Run 31 (plain
-O1, -A32m) **2.84x** over `bq-expand` paired, ahead on eighteen of nineteen
shapes. **That headline moves with the published REGIME and not with either arm,
and Run 31 prices the whole of it**: Run 29 read 2.22x on a `-fspec-constr`
...
alike, so that step was two evenings of drift on one build --- and this run
reads **2.84x** on the recipe again and **2.19x** on its `-O2` half. So raising
the level costs the headline better than six tenths of a multiple, `-O2`
speeding `bq-expand` by 29% and leaving the fill where it is, and the gap
...
section][floor], which owns it --- are in [Results](runs/run31.md#results), each
...
- [What is open](#what-is-open)
  - [Recommended tasks after Run 31](#recommended-tasks-after-run-31)
...
- [Run 31](runs/run31.md)
  - [Results](runs/run31.md#results)
  - [What the next run compares
    against](runs/run31.md#what-the-next-run-compares-against)
  - [The properties the next run should
    test](runs/run31.md#the-properties-the-next-run-should-test)
  - [The stride classes, run
    by run](runs/run31.md#the-stride-classes-run-by-run)
  - [Provenance](runs/run31.md#provenance)
  - [What this run was built to answer, and what it
    answered](runs/run31.md#what-this-run-was-built-to-answer-and-what-it-answered)
...
- `OPEN` **What Run 32 is built to answer, registered before it runs.**
  Registered 2026-09-14, before the run, on the pair declared in [Run 31's
  file](runs/run31.md#what-the-next-run-compares-against): Run 31's basis
  recipe, ghc-9.12.4 in the dead-spot form at plain -O1, as the basis, `nospec`
  still naming what that half is, against GHC HEAD at plain -O1 and nothing
  else, `ghead`, built through `cabal.project.ghead` as Run 19's and Run 27's
  HEAD halves were, so that `--compare` reads nospec over ghead and every
  `cross` below is what the compiler does to one arm at one level; over Run 31's
  roster plus `liblist-stage4-list-sum`, base's `sum` over stage four's list,
  landing 2026-09-14, and with the shared loop `sumLazyRuns` summing each run
  through `sumNoSpec` since the same day, a loop without vector's `SPEC`
  argument, which at -O1 reads -O2's counts on every runs shape, within
  a hundred instructions a call --- so the consumers carry -O2's counts on both
  halves and the two `-list-sum` arms, keeping vector's `sum`, are what price
  the argument. Items (1) to (14) are Run 31's as they stand in [that run's
  file](runs/run31.md#what-this-run-was-built-to-answer-and-what-it-answered):
  (4), (6) to (9) and (11) to (14) copied unchanged but for `ghead` written
  where they said `o2`; (1), (2), (3) and (10), whose `cross` bands
  were `-O2`'s, re-aimed at a compiler pair, their bands read off Run 27's
  9.12.4-against-HEAD JSONs with `--compare` on 2026-09-14; and (5), killed
  on Run 31, re-registered at what that run read, the cancellation on three
  `window` views included, so that a kill here says the reading did
  not reproduce. (15) is edited for the loop change and (16) is new, every
  ...
  and `predict: cross lib-stage2-lean 1.0 within 4%` --- Run 27's compiler pair,
  9.12.4 over HEAD, read the three at 0.9995, 0.9956 and 1.0197 on the main set,
  the lean fill the one of them past 1%, and its class readings of the leaf
  and of `lib-stage1` between 0.9693 and 1.0310; Run 31 read the three
  at 0.9956, 0.9972 and 0.9997 under `-O2`. Killed by any of the three past 2%
  on the main set, which would say HEAD reaches the shipped fill where neither
  -O2 pass nor the level did. (2) *The compiler on the reference.* On the main
  set: `predict: cross list 1.0 within 2%`; on every class:
  `predict: cross list 1.0 within 3%` --- Run 27's compiler pair read `list`
  at 0.9967 on the main set, inside the 0.7% differencing bar, and 0.9768
  to 1.0062 across the classes, where Run 25's read it 1.10% apart and outside
  the bar, `list` being the reference every table is read against. Killed
  by `list` outside 2% of 1 on the main set, which would say HEAD moves
  the reference by more than any compiler pair here has; and the two columns
  are differenced only where it sits inside the bar, which the two compiler
  pairs straddle. (3) *The compiler on the fused list.* The lazy stages'
  consumers fold one flat loop, `sumLazyRuns`, whose continuations are known
  and which sums each run through `sumNoSpec` since 2026-09-14, so the loop
  carries nothing a level would remove and a compiler is what is left to move
  it. On `runs`, `window` and `block`:
  `predict: cross libunord-stage7-sum 1.0 within 3%`; on `rev`, `bcast`,
  `bcastmid`, `scaled` and `compose`, where the route fills or is one block:
  `predict: cross libunord-stage7-sum 1.0 within 2%` --- Run 27's compiler pair
  read the consumer it then had, stage five's over the level-form walker,
  at 0.9963, 1.0127 and 1.0044 on those three and between 0.9945 and 1.0035
  on the five. Killed by the consumer past 3% on `runs` or `block` on either
  side of 1, which would say HEAD compiles the walker or its sum differently,
  the counts then saying which. (4) *The odometer over the table.*
  `liblist-stage4-sum` over `liblist-stage2-sum`, the lazy odometer's consumer
  against the port's base-offset table's. On `runs`, both halves:
  `predict: pair liblist-stage4-sum liblist-stage2-sum 0.47 within 25%`;
  ...
  consumer behind the table's past the floor on the `ghead` half, which would
  say the lazy list's lead over the branch's list is one regime's and
  ...
  halves:
  `predict: pair libunord-stage10-sum libunord-stage7-sum 0.55 within 20%`;
  on `bcastmid`, both halves:
  `predict: pair libunord-stage10-sum libunord-stage7-sum 0.57 within 20%`;
  ...
  halves:
  `predict: pair libunord-stage10-sum libunord-stage9-sum 0.96 within 5%` ---
  Run 31 reading 0.5497 and 0.5491, 0.5715 and 0.5686, 0.9557 and 0.9847,
  and on `window` the two reorderings CANCEL on three views, stage ten behind
  stage nine on both halves at 1.3666 and 1.3689 on `window-128x128-k7`, 1.2355
  and 1.2359 on `window-64x64-k1x9` and 1.0785 and 1.1985
  on `window-224x224-k3`, ahead past the floor on `window-28x28-k5`,
  `window-32x32-c64-k3` and `window-64x64-c16-k3` and within a percent
  on the two remaining views, which is what killed the item as Run 31 registered
  it and what this registration expects to reproduce. Killed by stage ten behind
  stage seven past the floor on both halves on any zero-stride view, which would
  say the two changes do not compose where both fire; or by any of the three
  cancelling `window` views not behind stage nine past the floor on both halves,
  or any of the three ahead views behind it past the floor on both, which would
  say Run 31's cancellation was that run's own. (6) *Stage ten where neither
  fires.* On `rev`, `scaled`, `runs` and `block`, both halves:
  ...
  *What the leaf fusion buys.* The shipped leaf against the family root, the one
  ...
  allocating cells on the main set with a worst disagreement of 1.6%, where
  `-O2` took `bq-expand` to 2.11x and `list` to 23.45x on Run 31. Killed by any
  level moving past 2% on the `ghead` half, which would say HEAD changes what
  an arm allocates and not only how fast it runs. (11) *The lean fill's
  unrolling and the leaf.* On the main set, both halves:
  ...
  no pair is tied. (15) *What the shared loop's SPEC-free sum leaves the list
  interface.* `libunord-stage10-list-sum` over `libunord-stage10-sum`, base's
  `sum` over stage ten's list against the harness's own fold through
  `sumLazyRuns`: the same route under both, and since 2026-09-14 differing
  in two things rather than one --- the user's fold is inlined into the route
  where the harness meets one loop compiled once for every stage, which is what
  Run 31 asked, and the user's fold sums each run through vector's `sum`, which
  carries the `SPEC` argument SpecConstr is meant to specialise away and at -O1
  re-passes it every element and once a run, where the harness's loop now sums
  through `sumNoSpec` and carries nothing. Run 31 read the pair at 1.0002
  on the main set and 1.0132, 1.0154, 1.0170, 1.0080 and 1.0133 on `runs`,
  `window`, `block`, `flip` and `small`, both arms then paying the argument;
  what the argument is worth is what `-O2` was worth to the harness's loop
  there, 1.0267 on `runs` and 1.0168 on `window` on that run, and the two
  multiply per shape into the figures below. On the main set, both halves:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.0 within 2%`,
  read at 1.0061; on `runs`:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.04 within 4%`,
  read at 1.0403, `runs-2` at 1.42 and `runs-3` at 1.14 carrying it and every
  shape from `runs-5` up inside 1.2%; on `window`:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.03 within 4%`,
  read at 1.0325; on `block`:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.02 within 3%`,
  read at 1.0171; on `small`:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.04 within 5%`,
  read at 1.0580, where the level's per-call constants are part of what `-O2`
  read and the loop alone is expected to buy less; on `flip`, `bcast`,
  `bcastmid` and `compose`:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.0 within 3%`,
  read at 1.0093, 1.0103, 1.0063 and 1.0044; on `rev` and `scaled`, where both
  consumers fold one vector and the argument is paid on a loop the add chain
  paces:
  `predict: pair libunord-stage10-list-sum libunord-stage10-sum 1.0 within 1%`,
  read at 1.0020 and 1.0040. Killed by the pair inside 1% of level on `runs`
  and `window` on both halves, which would say the SPEC-free sum bought
  the shared loop nothing a run's length shows; or by it past its band there
  on both halves, which would say the argument costs more in the run
  than the probe's counts and Run 31's cross say. (16) *The same fold
  on the lean route.* `liblist-stage4-list-sum` over `liblist-stage4-sum`,
  base's `sum` over stage four's list against the harness's fold through
  the same loop, the arm landing 2026-09-14 so that the `-list-sum` pair stands
  on both routes in every run from here. Stage four and stage ten walk one loop
  where the canonical axes are already in stride order, which Run 31's counts
  put at the `runs` and `block` classes, the two consumers counting alike there
  to a tenth of a percent; on every other population stage ten's sort
  or its zero-stride move gives the fold a different walk, the main set among
  them, and the counts part by up to 3.7x. So on `runs` and `block` this pair
  is registered at stage ten's figures, and elsewhere at stage ten's bands
  as the only prior: on the main set, both halves:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.0 within 2%`;
  on `runs`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.04 within 4%`;
  on `window`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.03 within 4%`;
  on `block`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.02 within 3%`;
  on `small`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.04 within 5%`;
  on `flip`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.0 within 3%`;
  on `rev` and `scaled`:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.0 within 1%`.
  On `bcast`, `bcastmid` and `compose` the routes part, stage ten moving
  a zero-stride axis outermost where stage four leaves it where the canonical
  view puts it, and no reading of stage four's list arm exists:
  `predict: pair liblist-stage4-list-sum liblist-stage4-sum 1.0 within 3%`,
  the band stage ten's pair read inside on Run 31. Killed by the pair inside 1%
  of level on `runs` and `window` on both halves, as (15) is; or by this pair
  and (15)'s parting past the floor on `runs` or `block` on both halves, which
  would say the two routes do not walk one loop where the counts say they do.
- `ANSWERED` **What Run 31 was built to answer, registered before it ran ---
  and what it answered.** The registrations, their kill conditions and their
  verdicts are [in Run 31's own
  file](runs/run31.md#what-this-run-was-built-to-answer-and-what-it-answered),
...
- `ANSWERED` **GHC HEAD compiles a `Ptr`-walking fill into an allocating one,
  ...
  **1.8842** of `-u2`'s instructions --- so the same source, the same shim,
  ...
  builds. **Run 27 read it at full budget and it holds on every axis
  it was registered on**, which is [the ceiling](#the-mutable-ceiling-taken)'s
  twenty-third reading and is kept there with the rest of the series. **Why
  it was worth the entry rather than a shrug** stands: the `Ptr` form is refused
...
- `OPEN` **Each -O2 pass changes what an arm ALLOCATES and not only how fast
  ...
  31's sixteen arms reads 1.0000, the fill family running 1.0380 to 1.0530,
  which is Run 29's shape entire. So three readings now exist and the question
  stands unchanged: nothing here says why an optimisation pass should move
  an allocation multiple at all.
...
- `ANSWERED` **`lib-stage2-disp` and `lib-stage2-lean` are NOT the same code
  ...
  on the same two binaries --- a criterion process apiece, the counts being
  `run-counts.sh`'s own fixed-iteration pair --- it reads **1.0142**
  and **1.0148** against floors of 1.05% and 0.83%. So the difference
  is instructions the two arms do not share rather than where they landed,
  and the halves no longer part: 1.0142 and 1.0148 in time, 1.0188 and 1.0180
  in counts, where Run 26 read 1.0171 against 1.0316. The ruling itself
...
- `ANSWERED` **Does list fusion pay in this harness, and what does the list
  ...
  and the harness rebuilt on the answer.** Not as the code stood: neither
  a `build`-form producer nor a `foldl'` consumer nor both together changed
  a byte per run, because the fold, partially applied to its step and to zero,
  was floated to the top level away from the `build` and the fusion rule never
  saw the pair. Written so the fold sits on the list expression itself,
  it fuses: 104 bytes and 14 ns a run against 145 and 19 in the probe, about
  a fifth of the time and a quarter of the allocation on every class where
  the list route fires, and 88 bytes a run once the fused loop was compiled once
  for every stage; a strict loop with no list reads 10 ns and nothing a run,
  about half the time. **DECIDED 2026-09-09**: `lazyRuns` is in `build` form;
  each lazy stage's dispatch is a value, `Route`, read by four shared readers,
  the list, the fill and one fused sum loop compiled once, so every stage's
  consumer is one code and a pair of stages prices dispatch alone; and the loop
  fold is one timed arm, `libunord-stage6-loop-sum`, for what the list interface
  costs a reduction. The probe arms are gone with their reading. Run 27's
  figures for `libunord-stage4`, `-stage5`, `liblist-stage3`, `-stage4`
  and the four `-sum` consumers are therefore not this roster's, nor, since
  the same day's ruling on the singleton copy, the six list Fill arms', the four
...
- `OPEN` **An argument that a reordering cannot reach a population
  ...
  dispatches parting where they were argued to be one route. **The converse does
  NOT hold and the count is what makes that worth keeping**: registrations (2),
  (4), (10) and (15) are no-op predictions too and all four held --- (2)
...
- `ANSWERED` **The A/A floor and the carry-back figure came apart on Run 28
  ...
  30 and again on Run 31 --- so which of them a margin between two rows must
  clear is a ruling and not a measurement, and four runs have now said so. RULED
  2026-09-13: the whole-set floor is the bar and the carry-back figure
  the series.** The shipped fill's own A/A copies landed 2026-09-09,
  so the floor is a maximum over EIGHT pairs and reads **0.61%** on Run 31's
...
- `OPEN` **A recurring transient that lands on the `bq-expand` family, worth 35
  ...
  **What follows for reading a table, before any of it is measured further.**
  ...
  3. **Winsorizing is a defence and not only an estimator choice.** It is what
     ...
     column](runs/run31.md#results) argues for it on estimator grounds ---
...
- `PARKED` **`mut-odo`'s wide interval on `micro-aligned` is, at sample level,
  ...
  **0.82**, against 0.31 on `micro-maxskip` --- so the split between
  ...
  ruling](#recommended-tasks-after-run-31)).
...
### Recommended tasks after Run 31
...
2026-09-13 into 09-14 and recorded in the note.** A CHECK THAT WOULD HAVE CAUGHT
AN ERROR, with a gap beside it: preflight's step 9 holds the BASIS binary
to the regime its own recipe asks for and reads the basis ALONE, so on a pair
whose variable IS the optimisation level nothing machine-checks the other half
--- what stands in is the note's own 9b, `diag` on the `-O2` half. AND THE STEP
ITSELF WAS WRONG: it derived the expected regime from a flag NAME searched
over the whole recipe block, so a basis built at -O2 would have FAILed for being
what its recipe asks and a block whose PROSE names a flag would have passed
as flagged. Repaired to read the `--ghc-options` lines alone at `dace8e7`
and recorded at `058a50f`, with the derivation now proved over five inputs
in preflight.sh's own header. A COMPUTATION IMPROVISED, two: the `.text` deltas
between the three binaries, and Run 30's twenty-two counts totals, which
the note's COUNTS block asks for leg by leg and no mode prints. A STEP SKIPPED:
none; 12a and 12c arrived DONE, the registration having landed
and been committed before the preparation began. A CAPABILITY FOUND, two:
`--carried` named item (15) and only (15), which is that mode asking the wrong
question BY DESIGN, and `diag` DOUBLES as 9b on this pair, so 9 and 9b are one
reading --- which no earlier pair here could say. AND ONE FOR THE NEXT
PREPARATION: `--draft` rewrote a bare tag inside an ANECDOTE about a previous
draft, a third case beside the roll and the live halves that the roll's backtick
rule does not reach.

**THE WRITE-UP SESSION'S HALF, and the first item is the one no gate would have
caught.** **A FIGURE WAS WRITTEN BEFORE IT WAS COMPUTED, AND ONLY THE WRITER'S
RE-READING CAUGHT IT.** A sentence added to Provenance said the three main-set
anchors move "0.32%, 0.35% and 1.53%" against `run30-nospec`; the figures
were invented to fit the sentence's shape and the real ones are -0.29%, +0.25%
and -1.45%. `--check-doc` cannot see it --- a plausible figure at a site
no other site quotes is checked by nothing here --- and the checker pass would
have found it only by recomputing that one clause. What caught it was reading
back the sentence just written, which is the one instrument this harness has
for that class of defect. **FOUR CHECKS DID FIRE AND EACH NAMED ITS SITE**, all
of them `--check-doc`'s cross-document agreement rules: the floor pair,
the carry-back figure and the cross-half `list` move each quoted at a stale
site, and the `Results section names run N` rule catching two cross-run binary
...
step gathers --- and it is TWO sessions' worth, the preparation's reaching
the executor only through the pair note.** **The preparation's half, taken
2026-09-07 and recorded in the note:** the check that caught an error
was preflight's 4,5, the byte-identical `check` comparison, which failed
...
step gathers --- and it is TWO sessions' worth, the preparation's reaching
the executor only through the pair note.** **The preparation's half, taken
2026-09-04.** THE CHECK THAT WOULD HAVE CAUGHT ITS ERRORS was pre-run step 12b
again, and it caught three, all in the note and none reachable by any mechanical
pass: `twelve A/A controls` where two of the twelve are `-nosum` forcing
controls, a `gate arms` line naming `run-gate.sh`'s five globs as they stood
before the prune re-cut them the same day, and a class-view delta of six where
it is eight. Each was found by re-deriving the figure from the binary
or the script it came from, which is what 12b asks and what nothing else does;
two more figures were transcribed rather than derived and came out true, which
is not the same as having been read. THE COMPUTATIONS IT IMPROVISED were two
and both have a mode now, written the same day: the class-view count, which
is neither `--list`'s bench total nor `classes --list`'s, and which
`./roster-delta.py PREV NOW` prints; and the fill displacements, which
`./loop-offsets.py --delta PREV NOW` subtracts. A STEP SKIPPED: none.
A CAPABILITY NOT USED: `./read-run.py --note PREV --draft R --halves b,o`, which
prints the previous note's `[SAME]` blocks with the names already substituted,
and which that session hand-copied instead --- the gate arms and the `--library`
figures came forward in the copying.

**The executing session's half, and the four are these.** **THE CHECKS
THAT WOULD HAVE CAUGHT THE ERRORS were mechanical this run, which is new.**
`--check-doc` refused seven distinct classes of defect in one command
over the course of the write-up --- a class-block count against `Main.hs`,
a class table's shape count, six class blocks quoting the previous run's floors,
four `[[TODO]]` markers, three dead anchors the moved registration carried,
the floor pair disagreeing across its three sites, and two shape counts quoted
as populations that `Main.hs` defines none of --- and every one of them
was this session's, made and caught inside the write-up. `install-tables.sh`'s
...
- `ANSWERED` **Runs never overlap in the benchmarked set.** `mkStrided`'s index
  ...
  classes, run by run](runs/run31.md#the-stride-classes-run-by-run) carries
  the figures.
...
1.83x on Run 23 and **1.83x** on Run 24, the figure moving three ten-thousandths
...
on Run 29's and 0.9532 on Run 28's basis. Run 31 times neither, so the ceiling
is recorded at those five readings and a run that wants a sixth has to un-park
an arm. What Run 31 does say about the form is from the naming side rather
than the timing one: post-run step 0 still finds `fbMutOdoVecdimsAddInLeafU2Ptr`
among the straddling loops of BOTH halves, parking having taken the arm off
the roster and not out of the binary.
...
explain: the [Results](runs/run31.md#results) columns and the [per-class
blocks](runs/run31.md#the-stride-classes-run-by-run). What is *not* here
...
not otherwise.
    ...
    #      the run file alone whatever else the commit carries. The
    #      commit is the first action and not housekeeping: given one
    #      commit's diff the pass has a fixed object, so 6c's edits cannot
    #      move under it and you need not stop writing. PRE-AUTHORIZED by
    #      the user-scope CLAUDE.md and not a thing to ask about.
...
file](runs/run31.md#results), 3 and 4 being [what the next run compares
against](runs/run31.md#what-the-next-run-compares-against), 5 [the
properties](runs/run31.md#the-properties-the-next-run-should-test) and 6 [the
class blocks](runs/run31.md#the-stride-classes-run-by-run) --- and `--section`
...
`bq-expand`, `mut-odo-vecdims` and `mut-odo-vecdims-add-in-leaf-u2`, each
with an adjacent and a distant twin, unmoved since the shipped fill's own copies
landed 2026-09-09 --- against Run 30's 0.57% and 0.84%, Run 29's 0.51% and 0.82%
and Run 28's 0.50% and 0.65% over the same eight, and Run 27's 0.83% and 0.63%
and Run 26's 0.31% and 0.46% over six, and Run 25's 0.40% and 0.61%, and against
Run 24's 1.26% and 2.11%, Run 23's 2.03% and 2.80%, Run 22's 2.12% and 1.08%
and Run 21's 2.92% and 2.16%, all over sixteen, and Run 20's 1.51% and 1.18%,
Run 19's 2.32% and 1.71%, Run 18's 1.36% and 1.42%, Run 17's 3.70% and 3.89%
and Run 16's 2.32% and 1.22%, every one of those five over eighteen. **A max
over six pairs, one over eight, one over sixteen and one over eighteen are four
different statistics**, so the sixteen-pair and six-pair figures are
not a series this run's whole-set figure continues; what it does continue
is the eight-pair one, whose four basis readings are now 0.50%, 0.51%, 0.57%
and 0.61%. The RESTRICTED figure, over the four pairs that carry back to Run 10,
reads **0.61%** and **0.43%**. `bq-expand-aa-distant` carries BOTH of the basis
figures and the control's restricted one,
`mut-odo-vecdims-add-in-leaf-u2-aa-distant` the control's whole-set figure ---
which is exactly the arrangement Run 30 read. **So the two thresholds are CLOSED
on the basis half for a second run**: 0.61% against 0.61%, the same pair
carrying each, where Run 28 read 0.50% against 0.39% and Run 29 0.51% against
0.26%. **On the control they part further than on any run since the prune,
...
19's finding**, there being no repetition: `Main.hs` moved three times
on 2026-09-13, so no binary here is an earlier one built again and no floor
...
**Where the movement lives is the one thing fourteen runs now agree on.**
The carry-back figure has read 0.54%, 0.49%, 0.44%, 0.46%, 0.37%, 0.39%, 0.34%,
0.40%, 0.31%, 0.83%, 0.39%, 0.26%, 0.57% and now 0.61% across those fourteen
...
moved. **Run 27's 0.83% still does not survive as a trend**, but this run's
0.61% is the second highest of the fourteen and sits above the 0.31%-to-0.54%
band the series held for nine runs --- so the band has been left in both
directions inside five runs, and quoting it as a bound is what this paragraph
stops doing. The threshold this run supports is TWO figures as Runs 28's, 29's
and 30's were --- **0.61% and 0.61%** on the basis, the restricted four-pair
reading and the whole set over the eight, which this run again makes one number
...
from OUTSIDE the declared pairs are still gone with their arms.**
...
of 2026-09-11, so this run, like Runs 29 and 30, has no check on the declared
...
then it was held to the carry-back figure of the half it is read on --- 0.61%
on Run 31's basis --- which is the A/A floor above restricted to the pairs
...
and each reads as two numerals because there are two halves**: 0.61% and 1.58%
...
to Run 10 give 0.61% and 0.43% --- equal to the whole-set figure on Run 31's
basis, as on Run 30's, where Runs 28 and 29 read the two apart, and the rule
since 2026-09-13 names the WHOLE-SET figure as what two rows of one table must
clear, the restricted one having been the rule until Run 30 re-opened it,
the wider figure being the conservative reading --- and 2.1% is the across-run
...
against](runs/run31.md#what-the-next-run-compares-against), those runs' own
...
file](runs/run31.md#provenance) and is replaced with the rest of it. What
...
- Run 31 measured TODAY's shapes and class views exactly and, less one arm,
  ...
  of `liblist-stage4-list-sum`, base's `sum` over stage four's list,
  and that day's change of the shared loop `sumLazyRuns` to sum each run through
  `sumNoSpec`, without vector's `SPEC` argument, a code change under unmoved
  names that reaches the `Route` stages' consumers wherever a view routes
  to runs, so today's roster is 32 timed arms, 608 benches and 1856 --- 31 timed
  arms over 19 main-set shapes and 58 class views in TEN classes, 589 benches
  and 1798, EIGHT A/A pairs, the `runs` class at FOURTEEN, `window` at EIGHT,
  ...
  file `runs/` currently publishes. **Its delta against RUN 30 is SIX ARMS OUT
...
- Run 28 measured TODAY's shapes and class views and, less ten arms and plus
  ...
  no figure; and, on 2026-09-13, the walker's bangs, which Run 30's bullet names
  and which reach the `Route` stages' lists and consumers wherever a view routes
  to runs --- so its figures need no adjustment outside those cells
  and the cells those bangs reach; and ten of its timed arms are not today's,
  `libunord-stage8-sum`, checked and not timed since 2026-09-11 and refuted
  at registration (6), `-u2-last`, `-u2-down` and `libunord-stage4-sum`, parked
  the same day on readings this run repeated, and the six parked 2026-09-13,
  which Run 30's bullet names, while `libunord-stage10-sum`, stage seven's
  tie-break under stage nine's move, is timed since that day and was not timed
  by this run, as `libunord-stage10-list-sum`, base's `sum` over stage ten's
  list, is timed since 2026-09-13 and was not --- 39 timed arms over 19 main-set
...
- Run 27 measured neither today's class views nor today's roster, and
  ...
  **and the nineteen main-set shapes are unmoved between the two runs,
  ...
  and no continuation of the sixteen-pair series.
- Run 26 measured neither today's roster nor the run `runs/` currently publishes
  ...
  both with their causes. **And its floor was a maximum over the same SIX A/A
  pairs Run 25 read**, reading 0.31% on its basis half and 0.46% on its control,
  tighter than Run 25's on both halves and no continuation of the sixteen-pair
  series.
...
- Run 25 measured the shapes, class views and roster of its own day --- 24 timed
  ...
  else --- contributes the second column of `runs/run25.md`. Its `list` moved
...
- Run 24 measured the shapes, class views and roster of its day --- 52 timed
  ...
  are named at the end of this bullet. **Its delta against RUN 23** is a roster
  and a shape-set delta both: `lib-stage2-short-lean` landed as a timed arm
  ...
  with no intrusion and no population rerun. **Its shape set and class list
  are not today's**, which is what makes a cross-run reading of it want pinning:
  the two window views `window-224x224-k3-s2` and `window-224x224-k3-d2`
  and the four classes `flip`, `block`, `small` and `compose` landed
  on 2026-09-03, after it ran, so it timed none of the eighteen views they
  brought; and `reshape1`, `revsome` and `slice` went out of the timed set
  on 2026-09-04 with the eight main-set shapes `stretch-inner1`,
  `lenet-slice-c6-k5`, `cnn-L1-6x6-c1`, `cifar-L2-16-c64-k3`, `stretch-rank10`,
...
- Run 23 measured Run 22's shapes, class views and roster, nothing having moved
  ...
  pairs, the `runs` class at ELEVEN of today's fourteen and `window` at three
  of four --- so its delta against RUN 22 is EMPTY and its delta against TODAY
  is Run 24's, in the bullet above: two main-set shapes and four class views
  landed after it ran, and its figures are over the population it had. More
  than empty against Run 22: its basis half, `run23-g912`, is Run 22's basis
  BINARY byte for byte, `Main.hs` having moved by one comment line and the shim
  by one commit that adds a switch that half does not set, so every distance
  between the two runs' basis columns is drift and nothing else, and it measured
  that drift at under 1% on 44 of 49 arms over the shapes the correction leaves
  readable. What a reader has to carry is which half a figure came from:
  everything published in its file is `run23-g912`, the max-skip half,
  and `run23-spot` --- the same recipe with `LOOP_DEADSPOT=1` in front
  of the shim, so the halves differ in where the pads sit and in nothing
  a freeze, a compiler or a source diff can see --- contributes the second
  column of `runs/run23.md`. Its `list` moved 0.33% between the halves, inside
  the 0.7% bar, so its two columns may be subtracted, which Run 22's could not.
  The box did not move, its gate machine check reading -0.44% and its main-set
...
- Run 22 measured today's shapes, today's class views AND today's roster,
  ...
  21: 55 timed arms over the 24 main-set shapes of the time and 37 class views
  in NINE classes, 1320 benches and 2035, where Run 21 ran 49 arms and 33 views,
  1176 and 1617. Six timed arms landed --- `lib-stage2-disp`, `lib-stage2-u4`,
...
- Run 21 measured today's shapes and class views, and its roster is today's LESS
  ...
  are read against today's fourteen only where a length is shared. Its delta
  against RUN 20 is larger again: 49 timed arms over the 24 main-set shapes
  of the time and 33 class views in NINE classes, 1176 benches and 1617, where
  Run 20 ran 53 arms and 26 views in eight classes, 1272 and 1378. Six timed
  arms landed (`mut-odo-vecdims-add-in-leaf-u2-down`, `lib-stage1`,
  `lib-stage2`, `lib-stage2-concat`, `liblist-stage1`, `liblist-stage2`) and ten
  names left --- eight arms parked permanently on 2026-08-28 (`bq-expand-b`,
...
- Run 20 measured today's main-set shapes but NOT today's class views --- 26
  in eight classes, where today's are 41 in nine --- and NOT today's roster read
  against Run 19's: nine timed arms landed and three dropped to `Only`,
  so its membership is 53 timed arms over the 24 main-set shapes of the time
  and 26 class views, 1272 benches and 1378, where Run 19 ran 47 and 1128
  over 24 and 24, and `reshape1` and `bcastmid` went to four shapes.
  **So the delta against Run 19 is a roster and it is not empty**, which every
  figure read across that boundary carries as a layout term besides drift, Run
  10 having priced a reorder at 12 to 14% on the two arms whose loop the shim
  rescues. Which half a figure came from is the other thing to carry: everything
...
- Run 8 measured today's shapes and today's class lists, on Run 12's roster
  **minus the eight arms written between them** (`bq-expand-gm-mulback`,
  ...
  compares against](runs/run31.md#what-the-next-run-compares-against)
...
- [the run's own file](runs/run31.md) ENTIRE, which is what makes it a file:
...
- [the recommended tasks after Run 31](#recommended-tasks-after-run-31), which
...
[open]: #what-is-open
[open-tasks]: #recommended-tasks-after-run-31
