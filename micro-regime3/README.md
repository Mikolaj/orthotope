...
section][floor], which owns it --- are in [Results](runs/run29.md#results), each
...
- [What is open](#what-is-open)
  - [Recommended tasks after Run 29](#recommended-tasks-after-run-29)
...
- [Run 29](runs/run29.md)
  - [Results](runs/run29.md#results)
  - [What the next run compares
    against](runs/run29.md#what-the-next-run-compares-against)
  - [The claims the next run should
    test](runs/run29.md#the-claims-the-next-run-should-test)
  - [The stride classes, run
    by run](runs/run29.md#the-stride-classes-run-by-run)
  - [Provenance](runs/run29.md#provenance)
  - [What this run was built to answer, and what it
    answered](runs/run29.md#what-this-run-was-built-to-answer-and-what-it-answered)
...
- `OPEN` **What Run 30 is built to answer, registered before it runs.**
  Registered 2026-09-12, before the run, on the pair the request of the same day
  decided: Run 29's control recipe, ghc-9.12.4 in the dead-spot form at plain
  -O1 with no `-fspec-constr`, as both halves, `nospec` as it is and `libcase`
  with `-fliberate-case` added to its `--ghc-options`, `nospec` the basis,
  so that `--compare` reads nospec over libcase and every `cross` below is what
  that flag does to one arm; over Run 29's roster unchanged, `Main.hs` standing
  at `7685375` as Run 29 ran it, so `run29-nospec` is the basis recipe's
  ...
  *The flag on the fused list.* The lazy stages' consumers fold one flat loop,
  ...
  0.8 there, which would say the list's lead over the fill is the flag's. (4)
  *The odometer over the table without the flag.* `liblist-stage4-sum`
  over `liblist-stage2-sum`, the lazy odometer against the port's base-offset
  table. On `runs`, both halves:
  `predict: pair liblist-stage4-sum liblist-stage2-sum 0.5 within 25%`;
  on `block`, both halves:
  `predict: pair liblist-stage4-sum liblist-stage2-sum 0.78 within 25%` --- Run
  28 reading 0.4951 and 0.5177, 0.7831 and 0.8018 under the flag. Killed
  by the odometer's consumer behind the table's past the floor on the `libcase`
  half, which would say the lazy list's lead over the branch's list
  is the flag's and not the design's. (5) *The list over the fill without
  the flag.* `libunord-stage5-sum` over `libunord-stage3-sum`, the fused list
  against the same route's fill. On `runs`, both halves:
  `predict: pair libunord-stage5-sum libunord-stage3-sum 0.57 within 15%`;
  on `block`, both halves:
  `predict: pair libunord-stage5-sum libunord-stage3-sum 0.45 within 15%` ---
  Run 28 reading 0.5681 and 0.5810, 0.4457 and 0.4460; on `rev`, `bcast`,
  `bcastmid`, `scaled` and `compose`, both halves:
  `predict: pair libunord-stage5-sum libunord-stage3-sum 1.0`, both routes
  filling or returning one block, the consumer summing what `fillRoute` wrote
  in either. Killed by the pair past the floor on both halves on any of the five
  fill classes; or by the list not ahead past the floor on `runs`
  on the `libcase` half. (6) *Stage ten where both of its changes fire,
  and where one does.* `libunord-stage10-sum` over `libunord-stage7-sum`
  on `bcast`, both halves:
  ...
  a zero stride. Killed by stage ten behind stage seven past the floor on both
  halves on any zero-stride view, or behind stage nine past the floor on both
  halves on any unstrided window view, which would say the two changes do
  not compose. (7) *Stage ten where neither fires.* On `rev`, `scaled`, `runs`
  and `block`, both halves:
  `predict: pair libunord-stage10-sum libunord-stage7-sum 1.0 within 3%` ---
  `zerosOutermost` hands its list back unchanged where no stride is 0, both
  of its branches doing so, the partition being the identity then,
  so `routeUnord10` is `routeUnord7` there to the byte and only the extra pass
  over the axes separates the dispatches. Killed by the pair past 3% on both
  halves on any of the four, which would say the pass is visible at a call
  of tens of microseconds. On `small` the span prints a figure and no verdict,
  read per view by hand: behind on the four views other than `small-bcast32`
  by the pass, as stage nine's read 1.18 to 1.40 over stage six on Run 28 ---
  `small-flat64`'s zero stride sits on a unit axis `canonView` drops,
  so its route is stage seven's --- and ahead on `small-bcast32`. (8)
  *`lib-stage1` against the lean fill with one fill under both.* Since
  ...
  dispatch fills, and Run 28's reading stands as the prediction:
  `predict: pair lib-stage2-lean lib-stage1 0.23 within 20%` on `runs`
  and `predict: pair lib-stage2-lean lib-stage1 0.42 within 20%` on `block`,
  both halves; killed by the lean fill not ahead past the floor on either, which
  would say the slice-and-concatenate route no longer loses on short runs. (9)
  *The floor with the fill family's pair for a second run.* On the main set
  and on every class, both halves:
  ...
  each inside the population's floor as Run 28's forty-four readings were; read
  beside them by hand whether the whole-set floor is the fill pair's again,
  0.50% on Run 28 against the carry-back figure's 0.39%, which is what decides
  the open question in the floor entry. Killed by the adjacent copy outside
  the floor on both halves on the main set. (10) *Claim 10 without the flag.*
  On the main set, both halves:
  `predict: pair mut-odo-vecdims-add-in-leaf-u2 mut-odo-vecdims 0.64 within 5%`,
  eight readings across four runs spanning nine thousandths under the flag.
  Killed by the pair outside 5% on the `libcase` half, which would say what
...
- `ANSWERED` **What Run 29 was built to answer, registered before it ran ---
  ...
  the flag on the shipped fill, HELD on the main set at 1.0001, 0.9945
  and 1.0044, with ten of its thirty-three class readings outside 1% on `bcast`,
  `bcastmid`, `block` and `small`; (2) the flag on the reference, KILLED, `list`
  reading 0.8788 on the main set and past the 0.7% differencing bar on all
  eleven populations; (3) the flag on the fused list, HELD at 0.9779 on `runs`,
  0.9839 on `block` and 0.9771 on `window`, read raw after a reader defect found
...
- `OPEN` **The A/A floor and the carry-back figure came apart on Run 28
  for the first time since the prune, they are further apart on Run 29,
  and which of them a margin between two rows must clear is still two answers.**
  The shipped fill's own A/A copies landed 2026-09-09, so the floor is a maximum
  over EIGHT pairs and reads 0.51% on the basis where the four pairs that carry
  back to Run 10 read 0.26%; on the control both read 0.82%,
  `bq-expand-aa-distant` being the widest pair there on both counts. Run 28 read
  0.50% against 0.39% and 0.65% on both, and Runs 25 to 27 had them collapsed
  and quoted one figure a half. The standing rule in [the floor section][floor]
  names the RESTRICTED figure; every verdict in `runs/run29.md` uses
  the whole-set pair, which is the conservative reading and the one a margin
  ought to clear if the point of a floor is that an arm disagrees with its own
  duplicate by that much. **What would settle it** is a ruling,
  not a measurement --- **and the measurement that would have informed it is now
  taken.** Run 28's entry asked whether the new pair's 0.50% was the fill
  family's own property or that evening's, and said a second run carrying
  the same eight pairs would say which: Run 29 is that run, it carries them,
  and `mut-odo-vecdims-add-in-leaf-u2-aa-distant` carries the whole-set floor
  on its basis half again, at 0.51%. So the gap is the pair's and
  not an evening's, it widened rather than closed, and what is left here
  is the ruling alone. **What a ruling has to decide, now that the evidence
  is in**: whether a margin clears the floor of the pairs that carry back, which
  keeps the series comparable across twelve runs, or the floor of every pair
  the roster carries, which is measured on the arm the library actually runs
  and is the larger of the two on both runs that have read them apart.
  **The rename this entry also carried is TAKEN and stays taken**, 2026-09-11:
  the figure is the carry-back figure, named for the pairs that carry back
  to Run 10 rather than for a population size, in this file, on `read-run.py`'s
  `--chapter` line and in its agreement row, and in the corpus case's expected
  text --- a cross-site agreement check keying on the SIZE of a population being
  exactly what a roster change breaks.
...
- `OPEN` **A recurring transient that lands on the `bq-expand` family, worth 35
  ...
  **What follows for reading a table, before any of it is measured further.**
  ...
  3. **Winsorizing is a defence and not only an estimator choice.** It is what
     ...
     column](runs/run29.md#results) argues for it on estimator grounds ---
...
- `PARKED` **`mut-odo`'s wide interval on `micro-aligned` is, at sample level,
  ...
  **0.82**, against 0.31 on `micro-maxskip` --- so the split between
  ...
  ruling](#recommended-tasks-after-run-29)).
...
- `ANSWERED` **Which of the `bq-expand`-centric claims retire, now that the fix
  ...
  at the foot of [the
  claims](runs/run29.md#the-claims-the-next-run-should-test), which owns
...
claims](runs/run29.md#the-claims-the-next-run-should-test) carry both. The tenth
...
- `ANSWERED` **Runs never overlap in the benchmarked set.** `mkStrided`'s index
  ...
  classes, run by run](runs/run29.md#the-stride-classes-run-by-run) carries
  the figures.
...
10](runs/run29.md#the-claims-the-next-run-should-test) at 0.6438 and 0.6467
...
explain: the [Results](runs/run29.md#results) columns and the [per-class
blocks](runs/run29.md#the-stride-classes-run-by-run). What is *not* here
...
claims](runs/run29.md#the-claims-the-next-run-should-test)). The slots that stay
...
- claim 4's controlled pair, `bq-scan-mulback` against
  ...
  list](runs/run29.md#the-claims-the-next-run-should-test) has been re-aimed
  onto them.
...
for nothing. Items 2 to 6 are [the last run's own file](runs/run29.md#results),
3 and 4 being [what the next run compares
against](runs/run29.md#what-the-next-run-compares-against), 5 [the
claims](runs/run29.md#the-claims-the-next-run-should-test) and 6 [the class
blocks](runs/run29.md#the-stride-classes-run-by-run) --- and `--section` takes
...
**On Run 29 the floor is 0.51% on the basis half and 0.82% on the control,
and it is the SECOND run read over EIGHT pairs.** The eight are `list`,
...
landed 2026-09-09 --- against Run 28's 0.50% and 0.65% over the same eight,
and Run 27's 0.83% and 0.63% and Run 26's 0.31% and 0.46% over six, and Run 25's
0.40% and 0.61%, and against Run 24's 1.26% and 2.11%, Run 23's 2.03% and 2.80%,
Run 22's 2.12% and 1.08% and Run 21's 2.92% and 2.16%, all over sixteen, and Run
...
over eighteen. **A max over six pairs, one over eight and one over sixteen
are three different statistics**, so the sixteen-pair and six-pair figures
...
twelve runs now agree on.** The carry-back figure has read 0.54%, 0.49%, 0.44%,
0.46%, 0.37%, 0.39%, 0.34%, 0.40%, 0.31%, 0.83%, 0.39% and now 0.26% across
...
nothing --- both lost a member to the parking of 2026-09-11, so this run has
no check on the declared eight from outside them at all. What Run 28 got
...
the carry-back figure of the half it is read on --- 0.26% on Run 29's basis ---
...
runs, and each reads as two numerals because there are two halves**: 0.51%
and 0.82% are the widest an arm differs from its own duplicate by on each half
over the eight pairs this roster carries, while the four pairs carrying back
to Run 10 give 0.26% and 0.82%, and the standing rule names the RESTRICTED
...
against](runs/run29.md#what-the-next-run-compares-against), those runs' own
...
file](runs/run29.md#provenance) and is replaced with the rest of it. What
...
- Run 8 measured today's shapes and today's class lists, on Run 12's roster
  **minus the eight arms written between them** (`bq-expand-gm-mulback`,
  ...
  compares against](runs/run29.md#what-the-next-run-compares-against)
...
- [the run's own file](runs/run29.md) ENTIRE, which is what makes it a file:
...
- [the recommended tasks after Run 29](#recommended-tasks-after-run-29), which
...
[open]: #what-is-open
[open-tasks]: #recommended-tasks-after-run-29
