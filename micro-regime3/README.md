...
- `OPEN` **What Run 28 is built to answer, registered before it runs.**
  ...
  and items (5), (8), (10) and (11) below are re-based on a filtered probe
  of the flat-walker build at a two-second budget, one process a class, item
  (12) on a probe of its own the same hour and item (7) on one of the fills
  build, whose fill its denominator takes on every zero-stride view, their JSONs
  in the same directory. GHC HEAD's Core of the old loop, `10.1.20260803`
  through `cabal.project.ghead`, had the same three objects in the same places;
  the flat walker's HEAD Core is unread, and Run 28's HEAD half is where
  it is first met. The overhaul's first form inlined each stage's list function
  into its consumer, and two of the six copies of one identical loop came out 8
  bytes and several ns a run dearer than the others, stage nine's by half
  on the k3 window --- per-copy code generation, which a pair of stages would
  have read as a design's cost; so the loop is compiled once, `sumLazyRuns`,
  and reached through each stage's `Route`, at which every consumer reads 88
  bytes a run and the same time on the same dims,
  ...
  of the reader's corpus, which is every JSON in this directory and which
  a filtered run fails by design. Each item names its populations and carries
  a prediction and a kill condition. (1) *The dispatch cost of the first
  canonicalization.* On `small`, both halves:
  `predict: pair libunord-stage6-sum libunord-stage5-sum 0.92 within 4%`;
  the Fill pair's 0.96 within 3%, registered before the retirement, is read
  on no run. Read per view beside it, by hand, the consumers: stage six ahead
  past the floor on `small-row96`, `small-patch-k5` and `small-bcast32`, behind
  past the floor on `small-flat64`, and inside the floor on `small-patch-r5`.
  Killed by stage six's consumer behind stage five's past the floor on both
  halves on the class, which would say the longer sort costs more than the pass
  it saves; or by `small-flat64` inside the floor on both halves, which would
  say the sort's length is not what the probe read. (2) *The tie everywhere
  else.* On `rev`, `bcast`, `bcastmid`, `window`, `scaled`, `runs`, `block`
  and `compose`, both halves:
  `predict: pair libunord-stage6-sum libunord-stage5-sum 1.0`, the two routes
  being the same code past the dispatch on every view and the probe reading
  0.997 on `compose`. The main set and `flip` are read raw by hand,
  the unordered arms' cells being their own forcing term there, as Run 27's
  items (6) and (7) read them. Killed by the pair past the floor on both halves
  on any of the eight, which would say the dispatch is visible at a call of tens
  of microseconds or that the routes are not the same code after all. (3)
  *Allocation.* `libunord-stage6-sum` reads the same allocation multiple
  as `libunord-stage5-sum` on every view, off the allocation column, neither
  dispatch allocating a result-sized buffer; killed by any view where the two
  multiples differ at the column's own precision. (4) *The tie-break where
  it cannot fire, stage seven over stage six.* Stage seven puts the larger
  extent innermost on equal absolute strides, so the run is the longest
  unit-stride axis. Equal absolute strides alias, so the tie exists only
  in a self-overlapping view: of the timed views the six unstrided `window`
  views and `small-patch-r5`, item (5)'s, and no other. On `rev`, `bcast`,
  `bcastmid`, `scaled`, `runs`, `block` and `compose`, both halves:
  `predict: pair libunord-stage7-sum libunord-stage6-sum 1.0`; the same span
  prints a figure and no verdict on `window` and `small`, and the main set
  and `flip` are read raw by hand as item (2) reads them. Killed by the pair
  past the floor on both halves on any of the seven, which would say
  the tie-break moved a route it cannot reach. (5) *The tie-break where
  it fires.* On `window` the runs go from the kernel's width to the output's: 3
  ...
  and `-d2`, 0.49 on `window-64x64-c16-k3` and 0.43 on `window-32x32-c64-k3`,
  0.72 as the class geomean --- the cheaper the run, the less the tie-break
  saves, and the run's cost has fallen twice since the account of nine
  nanoseconds a run this item first carried, at the leaf and then at the walker.
  As a class span on `window`, both halves:
  ...
  on both halves on any of the six unstrided views, which would say the per-run
  cost is not what the dilated control read; by it off stage six's past
  the floor on both halves on `-s2` or `-d2`, where the route is unchanged;
  or by it behind on `small-patch-r5` on both halves. (6) *The longest chain,
  stage eight over stage six and over stage seven.* Stage eight chooses the run
  as the longest contiguous one over every order of absorbing axes whose stride
  equals the run's length, leaves the other axes in stage six's order, and takes
  stage six's route where a stride is 0 or none is 1. Against stage six it moves
  what stage seven moves and `small-patch-r5` further, 16 for 8; against stage
  seven its run differs on `small-patch-r5` alone, an axis with the run's length
  sitting away from the run after sorting being impossible in an injective view,
  and its outer levels differ only where two of them tie on stride, the `window`
  views, where stage eight keeps stage six's order and stage seven the reverse,
  the run count moving on none. So on `rev`, `bcast`, `bcastmid`, `scaled`,
  `runs`, `block`, `window` and `compose`, both halves:
  `predict: pair libunord-stage8-sum libunord-stage7-sum 1.0`, and on `small`
  the class span prints a figure and no verdict, `small-patch-r5` read by hand:
  stage eight's consumer ahead of stage seven's past the floor, the runs halving
  on a call the per-run cost dominates. Killed by stage eight's consumer off
  stage seven's past the floor on both halves on any view but `small-patch-r5`,
  which would say the chain moved a route the argument says it cannot; or
  by it behind on `small-patch-r5` on both halves. (7) *Zero-stride axes
  outermost, stage nine over stage six.* Stage nine moves every zero-stride axis
  outermost on the list route, so a broadcast lists one real slice as many times
  as the axis is long where stage six falls to the fill; the fill route
  and every view with no zero stride are stage six's. On `rev`, `scaled`,
  `runs`, `block` and `window`, both halves:
  `predict: pair libunord-stage9-sum libunord-stage6-sum 1.0`, the main set
  and `flip` read raw by hand. Where a zero stride exists the reading is per
  view, by hand: stage nine ahead where the repeated slice is long --- the three
  older `bcast` views, 6400 elements eight times, 2000 elements 900 times
  and 900000 twice; `bcastmid-c32-cnn`, `bcastmid-primes`
  and `bcastmid-block150k`, the whole source as one slice of 5184, 2813
  and 450000 elements repeated 32, 89 and 4 times, the sort having merged
  the transposed block with what stands outside the zero-stride axis;
  ...
  allocating a run, ahead on the short slices too: the fills-build probe reads
  stage nine's consumer over stage six's at 0.45 on `bcastmid-b200k`,
  a 9-element block repeated 200000 times, 0.97 on `small-bcast32`, 32 runs
  of eight and the one zero-stride view where the lead is slight, 0.46
  on `compose-zero-mid`, 20000 runs of 90, and 0.58 to 0.70 on the long-slice
  `bcast`, `bcastmid` and `compose` views; `compose-scalar` unchanged, every
  stride 0. The `bcast` repeat ladder added for this run, `bcast-src8`,
  `bcast-src64` and `bcast-src512`, the same 1.8 million elements as 8 elements
  225000 times, 64 elements 28125 times and 512 elements 3515 times, was added
  as the rungs between which the repeated slice would meet the fill,
  and the probe reads no crossover: 0.48, 0.46 and 0.62. Killed by stage nine's
  consumer behind stage six's past the floor on both halves on any zero-stride
  view, `bcast-src8` and `small-bcast32` above all, which would say a per-run
  cost the walker no longer pays is back; or by it off stage six's past
  the floor on both halves on any population with no zero stride. On `small`,
  whose four other views have no zero stride and take stage six's route,
  the probe reads stage nine's consumer 1.16 to 1.36 behind, its dispatch's
  extra pass on a call under a microsecond, so that class prints a figure
  and no verdict. Read beside it off the allocation column: stage nine's
  consumer allocates neither the result nor, since the walker went flat,
  ...
  *The ceiling's consumer, `libunord-stage5-sum` over `libunord-stage3-sum`.*
  ...
  on `-k5`, 0.88 on `window-64x64-c16-k3` and 0.96 on `window-32x32-c64-k3`,
  0.54 on `-k7`, 0.59 on `-d2` and 0.48 on `-k1x9`, 0.71 as the class geomean,
  and on `small` level on the three views that fill, 0.85 on `small-patch-r5`
  and 0.57 on `small-row96`. Killed by the pair past the floor on both halves
  ...
  three and four through their routes and the fused loop stages four to nine
  ...
  where canonicalization drops the unit axis and stages three and four list 3584
  ...
  and within a fifth of a percent of 1 on `rev`, `bcast`, `bcastmid`, `scaled`
  ...
  `bcast` views with a long broadcast run, where Run 27 read the lean fill ahead
  already on the two views it had, the hoisted broadcast read being older
  than this day, and 1.00 on `bcast-tall-Mx2` where Run 27 read 1.20, 0.95
  as the class geomean; 0.95 on `compose-rev-bcast`, 0.92
  on `compose-slice-bcast` and 1.04 on `compose-zero-mid`, whose zero-stride
  level is the middle one and the one view the probe reads the lean fill behind
  on. The doubling is what moves `bcastmid`, the odometer's writes replaced
  by copies of growing size, `bcast` having its zero stride innermost, where
  no copy runs; the broadcast unroll is what brings `bcast-tall-Mx2` level.
  ...
  on `bcastmid-b200k` or `bcastmid-c32-cnn`, which would say the doubling does
  not beat the odometer's writes at a full budget; or by the pair moving
  from Run 27's reading past the floor on both halves on any population
  with no zero stride, read through `--bridge`, which the two changes cannot
  reach. Run 29 is to give `fbMutOdoVecdimsAddInLeafU2` the second unrolling,
  the library's leaf being what the change is for, which returns
...
- `OPEN` **The basis half carries the wider class floor, run after run,
  ...
  **r = +0.01** over that run's twenty-two processes, so the kill --- a negative
  ...
  for the reason above, and the four-process block is what would.
