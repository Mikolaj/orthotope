...
**The classes break property 2 in all nine, where Run 23 broke it outright in seven, and the arms that break it are the roster's new ones.** `libunord-stage2` leads `rev`, `revsome` and `reshape1` at 0.001, 0.000 and 0.000 of the fix, its one-block test firing on every view of those classes and collapsing them to a single slice, so it prices dispatch and not filling; `lib-stage2-short-lean`, the arm that landed this run, leads `bcast`, `slice` and `window` at margins of 39.79%, 28.42% and 63.79% against those populations' floors of 5.43%, 2.76% and 6.94%; `lib-stage2-lean` leads `bcastmid` by 47.28% against 2.80%; `lib-stage1`, the shipped route, leads `scaled` by 9.07% against 2.09%; and `lib-stage2-disp`, the arm task 9's probe re-cut to 2048, leads `runs` by 22.87% against 3.07%. **Five of the nine are led by a stage-two arm, one by stage one and three by the unordered entry point** --- where a run ago the leads were candidates and `mut-odo-vecdims` siblings. `scaled`'s is `lib-stage1`, which this file keeps apart from the stage-two family everywhere else, so it is counted apart here. Property 3 holds in all nine on both halves, and property 1 holds in all nine OF THE FIX --- which is what it is stated of; the library-shaped arms break it on `runs`, `lib-stage1` among them, and the claims section says so.
