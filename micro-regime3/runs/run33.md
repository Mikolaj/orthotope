...
**The widest cross-half figure in the file, 29.5% on one `runs` arm, is the basis FILE's and not the compiler's, read 2026-09-16 after this file was written.** `lib-stage2-lean-u1` reads **1.2954** across the halves over the fourteen `runs` shapes --- the basis slower on all fourteen of them, from 1.013 to 1.473 --- and its counted work reads **1.0000**, so `time/counts` is the whole 1.2954; no registration names that arm on that population, and its unrolled twin `lib-stage2-lean` sits at 1.0028 beside it. Read each half against Run 32's same half, HEAD's u1 moved on none of the fourteen shapes and the basis's moved 12 to 19 percent on every shape from `runs-7` up, in a binary whose u1 loops are byte-identical to Run 32's; a byte-identical copy of `run33-exit` then ran the `runs-16384` cell at Run 32's speed while the original stayed 15 percent slow, every extra cycle in the fill loop's own line at the same residue, and evicting the original's cached pages brought it level. The term is the physical frame the page cache held that one page in, from the build through every process of the evening --- [the placement section][floor] prices it and post-run step 4a now reads for it with `--half-movers` --- and the same frame is what the arm's cross-half figures on `block`, `flip`, `scaled` and `window` and 1.86 points of its main-set 1.0209 are made of, the basis having moved against Run 32 on each while HEAD's half moved under two points. The later sentences that read that arm's cross-half number as the compiler's each carry a clause pointing here. HEAD's half carries a frame term of its own, read the same day: `mut-odo-vecdims-add-in-leaf-u1` 22 percent slower than Run 32 on `compose-rev-bcast` and `compose-slice-bcast` and 12 on `scaled-super-r3` with counts level, its original 11 percent slower an iteration than a byte-identical copy on the box's `/tmp` on the first, and its executing loop's page shared with no timed arm. Copy-tested later against the copy on the `hugebin/` mount, on its worst cell of each population where it is the low extreme, the term is 8 percent an iteration on `compose-rev-bcast`, the original itself having read 5 percent faster than at the first test, 9 on `bcast-inner8` and 5 on `scaled-super-r3`, and absent on `flip-last-rows` and `window-128x128-k7` --- so the reading that 9.12.4 wins on the shipped leaf's un-unrolled form is HEAD's frame on `compose`, `bcast` and `scaled` and stands on `flip` and `window`.
...
**The straddling loops stand at EIGHT on each half, as they did on Run 32, and the offset-0 column is where the exit span shows.** `loop-offsets.py --survey` reads **305** self-loops of at most 64 B in the basis's own compiled code, 184 of them at offset 0 and 8 straddling, against **323** on the control, 195 at offset 0 and 8 straddling --- 323 and not the 325 this run first read, `--survey` having stopped counting a nop-headed body on 2026-09-16, which takes two loops from that half and none from the basis. Run 32's halves read 134 of 292 and 130 of 310 in that column under TODAY's tool --- that run published 311 on its control, the loop the nop-head refusal takes --- under a half either way; this run reads about three fifths on BOTH halves, which is the switch and is recorded without a mechanism. Within the pair its `--library` mode reads **136** self-loops in common in the LINKED libraries --- a different population from the survey's, which counts Main's own --- **11.0%** of them at the same offset in line and **65.4%** in the same straddle state --- RUN 32'S THREE FIGURES TO THE DIGIT, under a shim that moved and a source that moved, and this file offers no account of that either. **Post-run step 0's naming was taken off the binaries that were timed**, with two `-g3` twins built from the same two recipes: six of the basis's eight straddlers are named by byte identity --- `fillStage2Short`, `fbMutOdoVecdimsAddInLeafU2` twice, and its `Down`, `Last` and `Ptr` forms --- and three of the control's, one of those three off the OTHER half's twin, which is what giving both twins buys. The fill groups name the same six and two arms on each half. **And the count check no longer refuses the population comparison on the control half**: its twin holds 324 against the timed binary's 323, where before the refusal it held 324 against 325. The OTHER half's twin is still fewer, 306, so a name taken off that one rests on its own byte match alone --- which is where `fbMutOdoVecdimsAddInLeafU2Ptr` comes from on this half.
...
**What the class says:** properties 1, 2 and 3 hold on both halves --- `worst` 0.057, tiers at 1.00x, 1.13x and 21.32x, identical on the control --- and the family's ceiling and the best arm outside it TIE at 0.024, `lib-stage2-lean` against `mut-odo-vecdims-add-in-leaf-u2`, the bolding going to the ceiling. The lean fill is priced against `mut-odo-vecdims` at 0.7990 over 8 of 14 shapes at sign p 0.79, a margin of 20.10% against this class's **3.11%** floor (`list-aa-adjacent`), by far the widest floor of the ten and the reason margins here are read with more room than anywhere else. **THIS CLASS CARRIES THE RUN'S LARGEST CROSS-HALF FIGURE, THE COUNTS SAY IT IS NOT CODEGEN, AND IT IS THE BASIS FILE'S PAGE FRAME AND NOT THE COMPILER'S**: `lib-stage2-lean-u1` reads **1.2954**, the basis slower on all fourteen shapes, with a counted ratio of **1.0000** --- while the class geomean is 1.0184 and its counted geomean 1.0025 --- and against Run 32 the basis alone moved on it, 1.2849 to HEAD's 0.9947 (the `runs` paragraph at the head). No registration names that arm here. **Its two columns MAY be differenced**, `list` having moved 0.49 of a point. Registration (3) DIES on this population: its three consumers, predicted at 0.81 within 6%, read 0.9593, 0.9574 and 0.9583.
