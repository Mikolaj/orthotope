...
section](../README.md#per-shape-where-the-geomean-hides-the-ordering). Run 20
added `canon-vd`, `mid-copy` and `bcast-set` and kept `canon-memcpy-r2`
and `canon-full` out under the one-representative clause; the `mut-runs` column
went from these tables on 2026-08-28 with the arm's parking.
