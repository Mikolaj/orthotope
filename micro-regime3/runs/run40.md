...
(1) *The nest's instruction saving reaches the clock, and `lib-stage3-lean` moves further ahead of `lib-stage2-lean`.* **HELD on both halves.** The pair reads **0.9656** on the basis and **0.9564** on the control, the basis inside the band the item drew, from none of the saving reaching the clock at Run 39's 0.9790 to all of it at about 0.961, and the control just past its far end, where Run 39 read 0.9783; neither reading reaches the 0.99 that would say the saving did not reach the clock, nor crosses the 0.95 that would say the nest bought more than its instructions. The saving the prior predicted is the one that happened: this run's own sweep puts the pair at **0.9751** of the instructions raw on the basis, the prior's 0.975, and 0.9601 net of the forcing pass, so about 86% of the corrected instruction saving reaches the clock on the basis and all of it, 108%, on the control.
